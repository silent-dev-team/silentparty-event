<script setup lang="ts">
    ...
    let emit = defineEmits(['paied','cancled']);
    let sum = $ref("");
    let page = $ref(0);
    let _shown = $ref(probs.shown);
    sum = "0";
    watch(() => probs.shown, (val) => {
        _shown= val;
    });
  ...
  >
    <transition name="fade" mode="out-in" >
      ...
      <v-card class="wrapper" height="100vh" v-if="page == 0" :key="1">
        ...
        <v-btn @click="page = 1" flat color="green" :disabled="parseInt(sum)/100 < parseInt(requested)"
          >Checkout
          <v-icon
      ...
      <v-card v-else height="100vh" class="wrapper" :key="2">
        <div class="display">          <div class="table-row">
        ...
        <v-btn @click="page = 0" flat color="orange"
          >ändern
          <v-icon
            style="margin-left: 5px"
        ...
        ></v-btn>
        <v-btn @click="page = 0; emit('paied')" flat color="green"
          >Speichern
          <v-icon
            style="margin-left: 5px"
            icon="mdi-alpha-x-circle-outline "
